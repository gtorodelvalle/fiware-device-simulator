...
function getProbabilisticText(textProbabilities) {
  ...
  if (randomNumber <= textProbabilities[0][0]) {
    return textProbabilities[0][1];
  }
  for (var ii = 1; ii < textProbabilities.length - 1; ii++) {
    if (randomNumber > textProbabilities[ii][0] &&
      randomNumber <= (textProbabilities[ii][0] + textProbabilities[ii + 1][0])) {
