...
function validateDomain(simulationConfiguration, callback) {
  if (!simulationConfiguration.domain) {
    return callback(new fdsErrors.SimulationConfigurationNotValid('No domain configuration information ' +
      '(the \'domain\' property is mandatory)'));
  }
  if (!simulationConfiguration.domain.service) {
    return callback(new fdsErrors.SimulationConfigurationNotValid('No service in the domain configuration ' +
      'information (the \'domain.service\' property is mandatory)'));
  }
  if (!simulationConfiguration.domain.subservice) {
    return callback(new fdsErrors.SimulationConfigurationNotValid('No subservice in the domain configuration ' +
      'information (the \'domain.subservice\' property is mandatory)'));
  }
  callback();
}
  ...
  if (Array.isArray(simulationConfiguration.devices) && simulationConfiguration.devices.length) {
    ...
    if (isUltraLightHTTPDevice(simulationConfiguration.devices)) {
      ...
      if (isUltraLightHTTPDeviceWithNoAPIKey(simulationConfiguration.devices) &&
        !simulationConfiguration.iota.ultralight.http.api_key) {
        return callback(new fdsErrors.SimulationConfigurationNotValid('No API key in the UltraLight HTTP IoT Agent ' +
          'configuration information (the \'iota.ultralight.http.api_key\' property is mandatory if UltraLight HTTP ' +
      ...
      if (isUltraLightMQTTDeviceWithNoAPIKey(simulationConfiguration.devices) &&
        !simulationConfiguration.iota.ultralight.mqtt.api_key) {
        return callback(new fdsErrors.SimulationConfigurationNotValid('No API key in the UltraLight MQTT IoT Agent ' +
          'configuration information (the \'iota.ultralight.mqtt.api_key\' property is mandatory if UltraLight MQTT ' +
          'devices are include with no specific API key information)'));
      }
      ...
      if (isJSONHTTPDeviceWithNoAPIKey(simulationConfiguration.devices) &&
        !simulationConfiguration.iota.json.http.api_key) {
        return callback(new fdsErrors.SimulationConfigurationNotValid('No API key in the JSON HTTP IoT Agent ' +
          'configuration information (the \'iota.json.http.api_key\' property is mandatory if JSON HTTP devices are ' +
      ...
      if (isJSONMQTTDeviceWithNoAPIKey(simulationConfiguration.devices) &&
        !simulationConfiguration.iota.json.mqtt.api_key) {
        return callback(new fdsErrors.SimulationConfigurationNotValid('No API key in the JSON MQTT IoT Agent ' +
          'configuration information (the \'iota.json.mqtt.api_key\' property is mandatory if JSON MQTT ' +
          'devices are include with no specific API key information)'));
      }
    }
  }
