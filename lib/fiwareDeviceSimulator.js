 ...
 */
function emitResponse(error, request, response, body) {
  /* jshint camelcase: false */
  if (error) {
    eventEmitter.emit(
      'error',
...
function getRequestOptions(elementType, element, attributes) {
  /* jshint camelcase: false */
  var url, path, contentType, body, json, value;
  if (elementType === 'entity') {
    ...
    if (element.protocol === 'UltraLight::HTTP') {
      ...
      element.attributes.forEach(function(attribute) {
         ...
         * }
         */
        body = body.concat(attribute.object_id + '|' + value + '|');
      });
...
function update(elementType, element, attributes) {
  ...
  if (requestOptions) {
    request(requestOptions, function(err, response, body) {
      emitResponse(err, requestOptions, response, body);
