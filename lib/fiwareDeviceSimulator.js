...
function getDeviceAPIKey(device) {
  ...
  switch (device.protocol) {
    case 'UltraLight::HTTP':
      return device.api_key || configuration.iota.ultralight.http.api_key;
    case 'UltraLight::MQTT':
      return device.api_key || configuration.iota.ultralight.mqtt.api_key;
    case 'JSON::HTTP':
      return device.api_key || configuration.iota.json.http.api_key;
    case 'JSON::HTTP':
      return device.api_key || configuration.iota.json.mqtt.api_key;
  }
}
...
function getRequestOptions(elementType, element, attributes) {
  ...
  var options = {
    ...
    headers: {
      ...
      'Fiware-Service': configuration.domain.service,
      'Fiware-ServicePath': configuration.domain.subservice,
      'X-Auth-Token': (configuration.authentication && configuration.authentication.token) ?
        configuration.authentication.token : undefined
    },
    json: json,
    body: body
  };
