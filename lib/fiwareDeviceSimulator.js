...
function emitScheduled(schedule, elementType, element, attributes) {
  ...
  if (element.entity_name) {
    ...
    event2Emit.api_key = element.api_key;
...
function getMQTTTopic(device) {
  /* jshint camelcase: false */
  return '/' + device.api_key + '/' + device.device_id + '/attrs';
}
...
function getUltraLightPayload(device) {
  ...
  device.attributes.forEach(function(attribute) {
     ...
     * }
     */
    payload = payload.concat(attribute.object_id + '|' + value + '|');
  });
...
function getRequestOptions(elementType, element, attributes) {
  /* jshint camelcase: false */
  var url, path, contentType, body, json;
  if (elementType === 'entity') {
    contentType = 'application/json';
    json = true;
    if (configuration.contextBroker.ngsiVersion === '1.0') {
      path = '/v1/updateContext';
      url = 'https://' + configuration.contextBroker.host + ':' + configuration.contextBroker.port + path;
      var contextElement = {};
    ...
    if (element.protocol === 'UltraLight::HTTP') {
      ...
      url = 'http://' + configuration.iota.ultralight.http.host + ':' + configuration.iota.ultralight.http.port +
        '/iot/d' + '?i=' + element.device_id + '&k=' + element.api_key;
      json = false;
...
function update(elementType, element, attributes) {
  var mqttURL, mqttTopic, mqttPayload, mqttRequest;
  if (elementType === 'entity' || (elementType === 'device' && element.protocol === 'UltraLight::HTTP')) {
  ...
  } else if (element.protocol === 'UltraLight::MQTT') {
    mqttURL = configuration.iota.ultralight.mqtt.protocol + '://' +
      configuration.iota.ultralight.mqtt.host + ':' + configuration.iota.ultralight.mqtt.port;
    mqttTopic = getMQTTTopic(element);
    mqttPayload = getUltraLightPayload(element);
    mqttRequest = {
      url: mqttURL,
      topic: mqttTopic,
      payload: mqttPayload
    ...
    if (!mqttClient || !isMQTTClientConnected) {
      if (configuration.iota.ultralight.mqtt.user && configuration.iota.ultralight.mqtt.user) {
        mqttClient = mqtt.connect(
          mqttURL,
          {
            username: configuration.iota.ultralight.mqtt.user,
            password: configuration.iota.ultralight.mqtt.password
          }
        );
      } else {
...
function requestToken(callback) {
  ...
  request(tokenRequestOptions, callback);
...
function onTokenResponse(err, response, body) {
  ...
  if (err) {
    ...
    emitError(new fdsErrors.TokenNotAvailable('Authorization token could not be generated due to error (code: ' +
      (body && body.error && body.error.code) + ', title: ' + (body && body.error && body.error.title) + ', message: ' +
