...
/**
 * The progress information interval
 */
var progressInfoInterval;
...
/**
 * The date when the last info was sent
 */
var lastInfoSent;
 ...
 * @param  {Boolean} force Flag indicating if the progress information should be forced and emitted
 */
function emitProgressInfo(force) {
  var currentTime = new time.Date();
  if (!lastInfoSent) {
    lastInfoSent = currentTime;
  } else if (force || (currentTime - lastInfoSent) >= progressInfoInterval) {
    eventEmitter.emit(
      'progress-info',
      {
        ...
        realElapsedTime: new time.Date() - realFromDate,
        simulatedElapsedTime: new Date() - fromDate
      }
    );
    lastInfoSent = currentTime;
  }
...
function resolveValue(interpolator) {
  ...
  if (interpolator.indexOf('time-linear-interpolator(') === 0) {
    ...
    try {
      value = interpolate(interpolator, 'attribute-function-interpolator', configuration.authentication.token);
    } catch (exception) {
      emitError(exception);
    }
    return value;
  ...
  if (maximumNotRespondedUpdateRequests >= 0) {
    if ((updatesRequested - updatesResponded) > maximumNotRespondedUpdateRequests ||
      ...
      var reScheduledJob = scheduler.scheduleJob(
        new Date(Date.now() + delay), update.bind(null, elementType, element, attributes));
  ...
  fiwareDeviceSimulatorValidator.validateConfiguration(config, function(err) {
    if (err) {
      ...
      if (configuration.authentication) {
        if (configuration.authentication.retry) {
          process.nextTick(async.retry.bind(null, configuration.authentication.retry, requestToken, onTokenResponse));
          nextTick();
        } else {
