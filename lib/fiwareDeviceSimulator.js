...
function emitProgressInfo() {
  ...
  } else if ((currentTime - lastInfoSent) >= progressInfoInterval) {
    eventEmitter.emit(
      'progress-info',
      {
...
/**
 * Registers a new update requests
 * @return {Number} The number of updates requested
 */
function registerUpdateRequest() {
  updatesRequested++;
}

/**
 * Registers a new update response
 * @return {Number} The number of updates responded
 */
function registerUpdateResponse() {
  updatesResponded++;
}
...
function update(elementType, element, attributes) {
  if (maximumNotRespondedUpdateRequests) {
    if ((updatesRequested - updatesResponded) > maximumNotRespondedUpdateRequests) {
      return _.delay(update, delay, elementType, element, attributes);
    }
  }
